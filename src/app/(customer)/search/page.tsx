...
export default async function SearchPage({
  ...
  // Build a map: proId -> { minPrice, categoryCount }
  ...
  const prosWithSlots = await Promise.all(
    matchingPros.map(async (pro) => {
      const slots = await getProAvailableSlots(pro.id)
      return {
        pro,
  ...
  const visiblePros = prosWithSlots.filter(p => p.slotCount > 0)
    ...
    <main className="min-h-screen bg-background">
      ...
      <div className="px-5 pb-12 space-y-3">
        {visiblePros.length === 0 ? (
          ...
          visiblePros.map(({ pro, slotCount, startingPrice }) => (
            <ProCard
              key={pro.id}
              proId={pro.id}
